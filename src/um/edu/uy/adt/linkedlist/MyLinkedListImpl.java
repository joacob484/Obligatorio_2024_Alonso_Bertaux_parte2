...
import um.edu.uy.adt.queue.EmptyQueueException;
import um.edu.uy.adt.queue.MyQueue;
import um.edu.uy.adt.stack.EmptyStackException;
import um.edu.uy.adt.stack.MyStack;
...
public class MyLinkedListImpl<T> implements MyList<T>, MyQueue<T>, MyStack<T> {
    ...
    @Override
    public void set(int indice, T valor) { //funcion creada por nosotros
        Node<T> temp = first;
        int count = 0;
        while (temp != null) {
            if (count == indice) {
